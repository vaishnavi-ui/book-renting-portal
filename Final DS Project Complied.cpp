...
Riya Tendulkar and Vaishnavi Rathod*/
	...
	*/
	...
	string customers[10][7]={{"1","Diya","4653258765","diyaarora123","diyaroxx","4,Orchard Street","Kandivali"},
	...
	{"5","Pranay","56748937520","kingtopper","hatemeplease","41,Riddhima Circle","Kurla"},
	...
	*/
	...
	int graphDatabase[10][10]={
		{0,8,2147483647,3,4,2147483647,2147483647,2,2147483647,10},
		{8,0,7,2147483647,6,2147483647,1,3,2147483647,7},
		{2147483647,3,0,8,2147483647,8,2,2147483647,8,2147483647},
		{3,2147483647,8,0,2,2,4,2147483647,2147483647,5},
		{2,2147483647,2147483647,10,0,2147483647,9,2147483647,2,7},
		{10,2147483647,5,2,2147483647,0,2147483647,7,3,8},
		{2147483647,1,2147483647,4,2147483647,5,0,9,1,2147483647},
		{9,3,4,2147483647,2147483647,4,9,0,2147483647,6},
		{2,1,2,2147483647,2147483647,10,3,5,0,2147483647},
		{10,2147483647,6,2147483647,7,4,2147483647,3,6,0}
		};
	...
	*/
	...
	class Graph{
			...
			void setGraph()
			{
				...
				dijkstra(graphByUser,places,sellerLocality,locality,noOfPlaces1); //Calling Dijkstra

				//Displaying the graph
				cout<<"Do you want to display this graph (Enter y/n):";
				cin>>choice;
				if(choice=='y'||choice=='Y')
					BFS(0,graphByUser,places,noOfPlaces1);
			}//function setGraph() ends here


			//This function is the breadth first search to print the graph
			void BFS(int vertex,int graph[10][10],string places[10],int noOfPlaces1)
			{
			    int i; //'for' Loop
			    cout<<endl;
			    int visited[noOfPlaces1]={0}; //Declaring visited array

			    //Creating an object of the predefined queue function
			    queue <int> q;
			    visited[vertex]==1;
			    q.push(vertex);
			    while(!q.empty())
				...
				for(i=0;i<noOfPlaces;i++)
					...
					for(j=0;j<noOfPlaces;j++)
					{
						...
						//Compares the values: goes inside the 'if' only if new value is smaller than the original value and the path exists
							if(graph[vertex][j]+dij[j][1]<dij[vertex][1]&&graph[vertex][j]!=2147483647&&j!=vertex)
							{
	...
	class cartMenu
	{
		...
		void menu()
		{
			...
			for(;;)
			{
				cout<<endl<<"ENTER YOUR CHOICE FROM THE ABOVE MENU: ";
				cin>>choice;
				cout<<endl;
				if(choice==9)
				{
	...
	*/
	...
	void loginAccount()
		...
		for(int runLoop=0;runLoop<100;runLoop++){cout<<"~";}cout<<endl;
		for(int runLoop=0;runLoop<100;runLoop++){cout<<"~";}cout<<endl;
		cout<<endl<<"Saw that book in crossword and wanted to buy it, but the price tag seemed too heavy for you? Want to read ALOT but less pocket money?"<<endl;
		cout<<"THEN YOU ARE AT THE RIGHT PLACE!! PICK-A-BOOK Renting Portal will help you rent MANY books from book-lovers like you at a very low price!!!!"<<endl;
		...
		cout<<endl<<"FOR YOUR CONVENIENCE WE WILL PROVIDE YOU WITH THE DISTANCE AND PATH AND THEIR CONTACT NUMBER,YOU WILL HAVE TO TAKE TO REACH YOUR USER!"<<endl;
		for(int runLoop=0;runLoop<100;runLoop++){cout<<"~";}cout<<endl;
		...
		do{
		cout<<endl<<"Are you a registered user?(enter y or n): ";
		cin>>loginUser;
	...
	void showBooksMenu()
		...
		do
		{
			...
			cout<<endl<<"Enter the genre you would like to explore today?(Make sure all the entries match the ones shown above(This is a case sensitivecode)!): ";
			cin>>choiceOfGenre;
			for(int runLoop=0;runLoop<100;runLoop++){cout<<"-";}cout<<endl;
			...
			if(choiceOfGenre=="Crime"||choiceOfGenre=="Drama"||choiceOfGenre=="Fantasy"||choiceOfGenre=="Action"||choiceOfGenre=="Horror")
			{
				...
				cout<<"Please enter the codes of the books you want to buy and enter 'exit' to exit:"<<endl; //input of the code which should be added to cart
				for(i=0;addToCart!="exit";i++)
				{
					cin>>addToCart;
					amt=0;
			...
			{
				cout<<"OOPS YOU ENTERED A GENRE WHICH WE DO NOT HAVE!!!! PLEASE TRY AGAIN!"<<endl;
				choice='y';
			}

			cout<<"Do you wish to continue viewing books by genres?(Enter y or n):";
			cin>>choice;
		...
		int i;//'for' loop
		    ...
		    while(!s.empty()) //running till stack is empty
		    {
			        ...
			        if(visited[vertex]==0) //checking for visited
			        {
			            ...
			            for(i=0;i<NoOfPlaces;i++)
			            {
			                if(graphDatabase[vertex][i])//checking if path exists
			                s.push(i);
			            }
			        }
		    }
			...
			if(adminChoice==6)
				...
				switch(adminChoice)
				{
					...
					case 5:
						...
						cout<<"As per the customer who was logged in and the the details of the seller of the last book,the source and destination are:"<<endl;
						cout<<"Source: "<<locality<<endl;
						cout<<"Destination: "<<sellerLocality<<endl;
						cout<<endl;
						objOfGraph.setGraph();
						break;
					default:
